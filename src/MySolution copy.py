...
class MyClassifier(BaseEstimator, ClassifierMixin):  
    ...
    def train(self, trainX, trainY):
        def _solve_binary_problem(trainX, trainY) -> Tuple[np.ndarray, float]:
            ...
            np.fill_diagonal(G1[:, n_features + 1:], -1)
            
            # Second set of constraints (non-negativity of slack variables)
            G2 = np.zeros((n_samples, n_features + 1 + n_samples)) + 1e-8 # small value to avoid numerical issues
            np.fill_diagonal(G2[:, n_features + 1:], -1)
            
            # Combine constraints
            G = matrix(np.vstack([G1, G2]))
            h = matrix(np.hstack([
                -np.ones(n_samples),    # for first constraint
                1e-8 * np.ones(n_samples)     # for second constraint
            ]))
